<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
   ...
   <!-- Check if $chats is set and non-empty before rendering links -->
@if(isset($chats) && $chats->isNotEmpty())
@foreach($chats as $chat)
    <x-nav-link :href="route('chat.show', $chat->id)" :active="request()->routeIs('chat.show')">
        {{ __('Chat with ') }}{{ $chat->contact->name }} <!-- Assuming each chat is associated with a contact -->
    </x-nav-link>
@endforeach
@endif
    ...
    @if(isset($chats) && $chats->isNotEmpty())
@foreach($chats as $chat)
    <x-responsive-nav-link :href="route('chat.store', $chat->id)" :active="request()->routeIs('chat.store')">
        {{ __('Chat with ') }}{{ $chat->contact->name }} <!-- Assuming each chat is associated with a contact -->
    </x-responsive-nav-link>
@endforeach
@endif
