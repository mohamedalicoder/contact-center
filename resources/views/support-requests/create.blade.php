                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('support-requests.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>{{ old('description') }}</textarea>
                            <x-input-error :messages="$errors->get('description')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="call_id" :value="__('Call ID (Optional)')" />
                            <x-text-input id="call_id" class="block mt-1 w-full" type="text" name="call_id" :value="old('call_id')" />
